...
// ---- v0.14.0: deck keyboard, needs-you jump, duplicate tab, progress, find count ----------
{
  ...
  await evaluate("window.obpterm.activate(window.obpterm.tabs[0])");
  // A request on the pane you are LOOKING at is passed straight through by design, so the
  // focus switch has to have landed before one is injected — otherwise this races it.
...
// ---- v0.21.18: idle sessions are exited when the machine runs out of memory --------------
{
  ...
  await evaluate(`(() => {
    ...
    p.lastVisited = Date.now() - 20 * 60_000;     // long settled, but far too recent for the
    p.lastOutput = Date.now() - 20 * 60_000;      // timed path's own cutoff
  ...
  await evaluate(`(() => {
    const p = window.obpterm.panesOf(window.obpterm.tabs.at(-1))[0];
    p.eco = false;
    window.obpterm.lastPressureEco = 0;   // the cooldown is not what is being tested here
...
// ---- v0.21.20: the three dead wires ------------------------------------------------------
{
  ...
  await evaluate("window.obpterm.ecoSweep()");
  assert.equal(
    await evaluate("window.obpterm.panesOf(window.obpterm.tabs.at(-1))[0].eco"),
    false,
    "a session touched seconds ago is never exited, however big it is or however full the machine",
  );

  // Once it has settled, size decides — the 2.2 GB one goes before the 50 MB one.
  await evaluate(`(() => {
    window.obpterm.lastPressureEco = 0;
    for (const t of window.obpterm.tabs) for (const p of window.obpterm.panesOf(t)) {
      p.lastVisited = Date.now() - 20 * 60_000;
      p.lastOutput = Date.now() - 20 * 60_000;
    }
  })()`);
  await evaluate("window.obpterm.ecoSweep()");
  const big = await evaluate("window.obpterm.panesOf(window.obpterm.tabs.at(-1))[0].eco");
  assert.equal(big, true, "the 2.2 GB session is exited once it has actually gone quiet");

  // And it does not come straight back for more: the machine needs a moment to give it back.
  await evaluate(`(() => {
    const p = window.obpterm.panesOf(window.obpterm.tabs.at(-2))[0];
    p.eco = false; p.lastVisited = 0; p.lastOutput = 0;
    window.obpterm.config.eco_after_minutes = 0;   // the TIMED path is a separate policy
    window.obpterm.lastPressureEco = Date.now();   // state the cooldown outright, do not infer it
    window.obpterm.ecoSweep();
  })()`);
  assert.equal(
    await evaluate("window.obpterm.panesOf(window.obpterm.tabs.at(-2))[0].eco"),
    false,
    "a second sweep inside the cooldown takes nothing",
  );
